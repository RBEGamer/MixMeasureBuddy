import config
import machine
import ili934xnew, tt14, sh1106, ssd1306, st7789
...
class mmb_display():
    ...
    def display_instance_creator():
        ...
        if config.CFG_DISPLAY_TYPE == "ili934":
            ...
            SCR_ROT = 7
            _spi = machine.SPI(config.CFG_ST7789_SPIINSTANCE, baudrate=15625000, sck=machine.Pin(config.CFG_ST7789_SCK_PIN), mosi=machine.Pin(config.CFG_ST7789_MOSI_PIN), miso=machine.Pin(config.CFG_ST7789_MISO_PIN))
            disp = st7789.ST7789(_spi, config.SCR_WIDTH, config.SCR_HEIGHT, rotation=SCR_ROT, cs=machine.Pin(config.CFG_ST7789_CS_PIN))
            # TODO IMPLEMEN NEEDED FUNCTION
            #disp.set_color(ili934xnew.ILI9341.color565(255, 255, 255), ili934xnew.ILI9341.color565(0, 0, 0))
            #disp.display.set_font(tt14)
            return disp
